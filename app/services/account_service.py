...
class AccountService:
    @staticmethod
    def create_account(user_id: int, account_type: str, bank_name: str):
        user = UserRepository.get_user(user_id)
        if not user:
            raise NotFoundError("Usuário não encontrado")
        if not account_type.strip():
            raise BadRequestError("account_type não pode estar vazio")
        if not bank_name.strip():
            raise BadRequestError("bank_name não pode estar vazio")
        return AccountRepository.create_account(
            user_id=user_id,
    ...
    @staticmethod
    def update_account(account_id: int, account_type: str | None, bank_name: str | None):
        account = AccountRepository.get_account(account_id)
        if not account:
            raise NotFoundError("Conta não encontrada")
        ...
        return AccountRepository.update_account(
            ...
            account_type=account_type.strip() if account_type is not None else None,
